  ...
  try {
    const result = await runTransaction(async (tx) => {
      ...
      const srcResult = await tx.run(
        'MATCH (c:Cuenta {titular: $name}) RETURN c.saldo AS saldo',
        { name: from }
      );
      const srcBalance = toNumber(srcResult.records[0].get('saldo'));
      log.push({ step: 2, action: 'READ', detail: `Saldo de ${from}: ${srcBalance}` });

      if (srcBalance < amount) {
        throw new Error(`Fondos insuficientes: ${from} tiene ${srcBalance}, necesita ${amount}`);
      }

      await tx.run(
        'MATCH (c:Cuenta {titular: $name}) SET c.saldo = c.saldo - $amount',
        { name: from, amount: neo4j.int(amount) }
      );
      log.push({ step: 3, action: 'DEBIT', detail: `${from} debitado: -${amount}` });

      await tx.run(
        'MATCH (c:Cuenta {titular: $name}) SET c.saldo = c.saldo + $amount',
        { name: to, amount: neo4j.int(amount) }
      );
      log.push({ step: 4, action: 'CREDIT', detail: `${to} acreditado: +${amount}` });

      log.push({ step: 5, action: 'COMMIT', detail: 'Transacción confirmada (COMMIT)' });

      const finalResult = await tx.run('MATCH (c:Cuenta) RETURN c.titular AS titular, c.saldo AS saldo ORDER BY c.titular');
...
app.post('/api/acid/transfer-with-crash', async (req, res) => {
  ...
  try {
    await runTransaction(async (tx) => {
      ...
      // Simulación de crash antes del crédito
      log.push({ step: 4, action: 'CRASH', detail: '⚠ ERROR simulado antes de acreditar a destino' });
      throw new Error('CRASH_SIMULADO: Fallo del sistema antes de completar la transferencia');
    });
  } catch (err) {
    log.push({ step: 5, action: 'ROLLBACK', detail: 'Transacción revertida automáticamente — saldos sin cambios' });
  }

  // Leer saldos finales para demostrar que no cambiaron
  try {
    const balances = await runQuery('MATCH (c:Cuenta) RETURN c.titular AS titular, c.saldo AS saldo ORDER BY c.titular');
...
app.get('/api/acid/balances', async (req, res) => {
  ...
  try {
    const result = await runQuery('MATCH (c:Cuenta) RETURN c.titular AS titular, c.saldo AS saldo ORDER BY c.titular');
...
app.post('/api/acid/concurrent-read', async (req, res) => {
  ...
  try {
    // Lectura concurrente: lanzamos transferencia y lectura en paralelo
    ...
    // Lectura desde otra sesión mientras la transferencia está en vuelo
    const readPromise = runReadTransaction(async (tx) => {
      ...
      const result = await tx.run('MATCH (c:Cuenta) RETURN c.titular AS titular, c.saldo AS saldo ORDER BY c.titular');
      const balances = result.records.map(r => ({ titular: r.get('titular'), saldo: toNumber(r.get('saldo')) }));
      log.push({ step: 'R2', action: 'READ_RESULT', detail: `Lectura vio: ${balances.map(b => `${b.titular}=${b.saldo}`).join(', ')}` });
      return balances;
    });
    ...
    log.push({
      step: 5,
      action: 'ISOLATION',
      detail: `La lectura concurrente siempre ve un estado consistente (total=${readTotal}). ACID garantiza aislamiento.`,
    });
